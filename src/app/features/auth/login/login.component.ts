...
@Component({
  selector: 'app-login',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, GlassPanelComponent, TranslatePipe],
  template: `
      ...
      <app-glass-panel>
        <div class="w-full max-w-sm p-4">
          <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-primary">{{ 'APP_TITLE' | translate }}</h1>
            <p class="text-text-color-secondary">{{ 'APP_SUBTITLE' | translate }}</p>
          </div>
          <form [formGroup]="loginForm" (ngSubmit)="onSubmit()" class="space-y-6">
            <div>
              <label for="userType" class="block text-sm font-medium text-text-color">{{ 'LOGIN_SIMULATE' | translate }}</label>
